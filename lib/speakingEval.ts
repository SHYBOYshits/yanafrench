// Server-only. Shared Gemini call that scores a recorded French answer —
// used by /api/speaking/evaluate (Speaking Practice) and /api/quiz/submit
// (the speaking question in a Quiz session), so both go through the same
// evaluation logic instead of drifting apart.
...
export class SpeakingEvalRateLimitError extends Error {}

export async function evaluateSpeakingAudio(prompt: string, audioBuffer: Buffer, mediaType: string): Promise<SpeakingEvaluation> {
  try {
      ...
      messages: [
        {
          ...
          content: [
            ...
            { type: "file", mediaType, data: audioBuffer },
          ],
        },
      ],
      output: Output.object({ schema: speakingEvaluationSchema }),
    });
    return output;
  } catch (error) {
    const cause = RetryError.isInstance(error) ? error.lastError : error;
    if (APICallError.isInstance(cause) && cause.statusCode === 429) {
      throw new SpeakingEvalRateLimitError("Rate limited");
    }
    throw error;
  }
}
